...
namespace crm_dotnet.Models.outil
{
    public class DashboardManager
    {
        ...
        public double getSommeBudgetByAnnee(int annee)
        {
            double result = 0;
            for (int i = 0; i < this.Budgets.Length; i++)
            {
                if(this.Budgets[i].Date_ajout.Year == annee)
                {
                    result += this.Budgets[i].Montant;
                }
            }
        ...
        public double getSommeLeadDepenseByAnnee(int annee)
        {
            double result = 0;
            for (int i = 0; i < this.LeadDepenses.Length; i++)
            {
                if (this.LeadDepenses[i].Date_ajout.Year == annee)
                {
                    result += this.LeadDepenses[i].Montant;
                }
        ...
        public double getSommeTicketDepenseByAnee(int annee)
        {
            double result = 0;
            for (int i = 0; i < this.TicketDepenses.Length; i++)
            {
                if (this.TicketDepenses[i].Date_ajout.Year == annee)
                {
                    result += this.TicketDepenses[i].Montant;
                }
            }
